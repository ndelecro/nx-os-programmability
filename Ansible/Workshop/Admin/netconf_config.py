...
def main():
    ...
    # nxos patch
    nxos_device = res = [i for i in capabilities.get('server_capabilities') if 'cisco-nx-os-device' in i]
    if nxos_device:
        nfilter = '<filter><System xmlns="http://cisco.com/ns/yang/cisco-nx-os-device"/></filter>'
    ...
    try:
        ...
        if source:
            ...
            if before is None:
               if nxos_device:
                   before = to_text(conn.get_config(source=target, filter=nfilter), errors='surrogate_then_replace').strip()
               else:
            ...
            if supports_commit and module.params['commit']:
                if nxos_device:
                    after = to_text(conn.get_config(source='candidate', filter=nfilter), errors='surrogate_then_replace').strip()
                else:
            ...
            if after is None:
                if nxos_device:
                    after = to_text(conn.get_config(source='running', filter=nfilter), errors='surrogate_then_replace').strip()
                else:
